    ...
    operations: {
      ...
      items: {
        ...
        properties: {
          ...
          y: { type: 'number' },
          z: { type: 'number' },
          w: { type: 'number', description: 'width, east-west, in feet. A size, never a position.' },
          d: { type: 'number', description: 'depth, north-south, in feet. "12 by 16" means w:12, d:16. Never put a size in x or y.' },
          h: { type: 'number' },
          level: { type: 'number' },
...
export function applyDeterministicRescues(plan, sourceSpec) {
  ...
  // NOTE: no re-tiling here. A banded shelf-tiler once re-placed all traced
  // rooms from the reader's positions ("Phase 2") — it satisfied every
  // invariant and flattened every real plan into rows the drawing never had
  // (a deep living room beside two stacked bedrooms is not expressible in
  // rows). Daniel: "the model looks nothing like the drawings." The reader's
  // positions ARE the drawing's massing; the rescues below legalize them
  // (overlaps slide apart, strays come inside) without redesigning the plan.
...
export function describeModelForAudit(spec) {
  const s = spec.shell || {};
  const lines = [];
  lines.push(`Shell: ${s.widthFt}x${s.depthFt} ft, roof ${s.roofType || 'gable'}${s.roofType === 'shed' ? (Number(s.eastWallHeightFt) && Number(s.westWallHeightFt) && Math.abs(s.eastWallHeightFt - s.westWallHeightFt) >= 0.05 ? ` (east wall ${s.eastWallHeightFt} ft, west wall ${s.westWallHeightFt} ft)` : ` (south wall ${s.southWallHeightFt} ft, north wall ${s.northWallHeightFt} ft)`) : ` (wall height ${s.wallHeightFt} ft)`}, storeys ${s.storeys || 1}${Number(s.basementHeightFt) > 0 ? `, basement ${s.basementHeightFt} ft` : ''}.`);
...
SHED DRAINAGE: a shed roof drains toward its LOW eave, along EITHER axis. Falls north/south: set_roof_profile roofType 'shed' with DIFFERENT southWallHeightFt/northWallHeightFt (high south draining north is the solar classic). Falls east/west: set_wall_height wall 'east' then wall 'west' with different h values (naming an east/west height moves the fall to that axis; naming south/north moves it back). Equal heights = a flat roof that won't drain; never emit that.
