...
# Units: millimeters. Permit-track coordination model with structural/code fields.
import json
...
doc = App.newDocument(SPEC["projectName"].replace(" ", "_"))
site_group = doc.addObject("App::DocumentObjectGroup", "Site")
building_group = doc.addObject("App::DocumentObjectGroup", "Building")
level_group = doc.addObject("App::DocumentObjectGroup", "Level_01")
site_group.addObject(building_group)
building_group.addObject(level_group)

shell = SPEC["shell"]
width = ft(shell["widthFt"])
depth = ft(shell["depthFt"])
wall_h = ft(${modeledRoof.highWallHeightFt})
south_wall_h = ft(${modeledRoof.southWallHeightFt})
north_wall_h = ft(${modeledRoof.northWallHeightFt})
wall_t = ft(${wallProfile.thicknessFt})
pad_ext = ft(shell.get("padExtensionFt", 4))
slab_t = 152.4
wall_color = (${wallProfile.rgb.join(', ')})
...
def add_box(name, x, y, z, sx, sy, sz, color):
    ...
    level_group.addObject(obj)
    return obj

add_box("Insulated_Slab", -pad_ext, -pad_ext, 0, width + pad_ext * 2, depth + pad_ext * 2, slab_t, (0.64, 0.62, 0.56))
add_box("${wallProfile.key}_North_Wall", 0, 0, slab_t, width, wall_t, north_wall_h, wall_color)
add_box("${wallProfile.key}_South_Wall", 0, depth - wall_t, slab_t, width, wall_t, south_wall_h, wall_color)
add_box("${wallProfile.key}_West_Wall", 0, 0, slab_t, wall_t, depth, wall_h, wall_color)
add_box("${wallProfile.key}_East_Wall", width - wall_t, 0, slab_t, wall_t, depth, wall_h, wall_color)

for room in SPEC["rooms"]:
    room_obj = add_box("RoomZone_" + room["name"].replace(" ", "_").replace("/", "_"), ft(room["x"]), ft(room["y"]), slab_t + 8, ft(room["w"]), ft(room["d"]), 35, (0.28, 0.48, 0.62))
    room_obj.Label = room["name"] + " zone"

for element in SPEC.get("elements", []):
    elem_obj = add_box("Element_" + element["name"].replace(" ", "_").replace("/", "_"), ft(element["x"]), ft(element["y"]), slab_t + ft(element.get("z", 0)) + 20, ft(element["w"]), ft(element["d"]), ft(element.get("h", 1.2)), (0.45, 0.38, 0.25))
    elem_obj.Label = element["name"] + " - " + element.get("category", "natural element")

roof = doc.addObject("Part::Feature", "${modeledRoof.roofType === 'shed' ? 'Shed_Lean_To_Roof' : 'Simple_Gable_Roof'}")
${modeledRoof.roofType === 'shed' ? `points = [
    App.Vector(-609.6, -609.6, slab_t + north_wall_h + 85),
    App.Vector(width + 609.6, -609.6, slab_t + north_wall_h + 85),
    App.Vector(width + 609.6, depth + 609.6, slab_t + south_wall_h + 85),
    App.Vector(-609.6, depth + 609.6, slab_t + south_wall_h + 85),
]
faces = [
    Part.Face(Part.makePolygon([points[0], points[1], points[2], points[3], points[0]])),
]` : `roof_rise = depth * float(shell["roofPitch"])
points = [
    App.Vector(-609.6, -609.6, slab_t + wall_h),
    App.Vector(width + 609.6, -609.6, slab_t + wall_h),
    App.Vector(width + 609.6, depth / 2, slab_t + wall_h + roof_rise),
    App.Vector(-609.6, depth / 2, slab_t + wall_h + roof_rise),
    App.Vector(-609.6, depth + 609.6, slab_t + wall_h),
    App.Vector(width + 609.6, depth + 609.6, slab_t + wall_h),
]
faces = [
    Part.Face(Part.makePolygon([points[0], points[1], points[2], points[3], points[0]])),
    Part.Face(Part.makePolygon([points[3], points[2], points[5], points[4], points[3]])),
]`}
roof.Shape = Part.Compound(faces)
roof.ViewObject.ShapeColor = (0.28, 0.31, 0.32)
level_group.addObject(roof)

for opening in SPEC["openings"]:
    marker_name = (opening["type"] + "_" + opening["label"]).replace(" ", "_")
    if opening["wall"] in ["north", "south"]:
        x = ft(opening.get("x", 0))
        y = -152.4 if opening["wall"] == "north" else depth
        sx = ft(opening.get("widthFt", 3))
        sy = 152.4
    else:
        x = width if opening["wall"] == "east" else -152.4
        y = ft(opening.get("y", 0))
        sx = 152.4
        sy = ft(opening.get("widthFt", 3))
    add_box("OpeningMarker_" + marker_name, x, y, slab_t + 914.4, sx, sy, 1219.2, (0.15, 0.42, 0.70))

doc.recompute()
doc.saveAs(SPEC["projectName"].replace(" ", "_") + "_Rev" + str(SPEC["revision"]) + ".FCStd")
`;
}
...
function createIfcSummary(spec) {
  ...
  return {
    ...
    revision: spec.revision,
    units: 'feet in UI, millimeters in FreeCAD export',
...
function App() {
  ...
  async function exportFreeCad() {
    try {
      const response = await fetch('/api/freecad/export', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ bim: spec })
      });
      if (!response.ok && response.status !== 202) throw new Error(`FreeCAD export failed with HTTP ${response.status}`);
      ...
      setChatMessages((items) => [
        ...items,
        {
          role: 'studio',
          speaker: 'Studio',
          text: `FreeCAD export ${result.ok ? 'completed' : 'prepared with warnings'}.\n\nScript: ${result.scriptPath || 'not available'}\nFCStd: ${result.fcstdPath || 'not available'}\nObjects: ${result.objectCount || 0}${result.warnings?.length ? `\nWarnings:\n- ${result.warnings.join('\n- ')}` : ''}`
        }
      ]);
      setOperationAudit((items) => [
        {
          id: `${Date.now()}-freecad`,
          prompt: 'FreeCAD export',
          source: 'freecadcmd',
          intent: 'export authoritative BIM model',
          beforeRevision: spec.revision,
          afterRevision: spec.revision,
          changedIds: [],
          operations: [`Generated script: ${result.scriptPath}`, `Generated FCStd target: ${result.fcstdPath}`],
          warnings: result.warnings || []
        },
        ...items
      ].slice(0, 60));
    } catch (error) {
      downloadFile(`generate-house-bim-rev-${spec.revision}.py`, createFreeCadScript(spec), 'text/x-python');
      setChatMessages((items) => [
        ...items,
        { role: 'studio', speaker: 'Studio', text: `FreeCAD server export was unavailable, so I downloaded the generated FreeCAD Python script instead.\n\n${error.message}` }
      ]);
    }
  }
      ...
      <section className="workspace">
        <header className="topBar">
          <div className="projectIdentity">
            ...
            <p>Revision {spec.revision} · {spec.shell.widthFt}' x {spec.shell.depthFt}' · FreeCAD/BIM export ready</p>
            ...
            {inspectorView === 'outputs' && (
              <div className="outputsPane fullPane">
                <div className="outputCards">
                  ...
                  <button className="secondary" onClick={exportFreeCad}><Hammer size={18} /><span><b>FreeCAD</b><small>Scripted BIM model</small></span></button>
                </div>
